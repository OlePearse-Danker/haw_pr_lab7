...
// dynamically allocating space for the struct
    ...
    fgets(buffer, 80, csv); //liest einmal eine Zeile aus

    while (fgets(buffer, 80, csv) != NULL)        // csv -> telling the fgets where to get the string from //
    {
        // realloc(coursePtr, (i + 1) * sizeof(course));                                       // neues Listenelement anlegen
    ...
    {
        printf("%s\t", courses[i].abbreviation);
        courses[i].grade = get_short("Note", 0, 15);                            // hier fragen wie man bei get_short noch eine Modulabk�rzung hinzuf�gen kann ()
        // printf("%d\n", courses[i].grade);

        product_sum = (courses[i].grade * courses[i].valuation) + product_sum;     // Zähler
        valuation_sum = courses[i].valuation + valuation_sum;                    // Nenner

    }

    float final_grade = product_sum / valuation_sum;
    printf("Abschlussnote: %.3lf\n", final_grade);
