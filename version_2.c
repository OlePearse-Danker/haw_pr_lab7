...
int get_grades(course x);                          // function to get the grades of the user
...
int main (void)
{
    ...
    {      
        course* ree = courses + 1;           // pointer 
                                             // fgets returns a NULL when it gets to the end (we continue as long as we are not at the end of the line)
        // saving module
        coursePtr = strtok(buffer, ";");   // Funktion sucht in der Zeichenkette 'buffer' nach einem Semikolon und setzt überall dort, wo es ein Semikolon findet die Null-Terminierung rein
        strcpy(ree->module, coursePtr);     // the first token is already a string so we can just copy it into our struct variable ree.module
        ...
        strcpy(ree->abbreviation, coursePtr);
    
        // saving valuation
        coursePtr = strtok(NULL, ";");
        ree->valuation = atoi(coursePtr);  // converting string to integer value

        coursePtr = strtok(NULL, ";");
        ree->grade = atoi(coursePtr);
    ...
    {
        courses[i].grade = get_short("Note", 0, 15);                            // hier fragen wie man bei get_short noch eine Modulabkürzung hinzufügen kann ()
        printf("%d\n", courses[i].grade);
        
        product_sum = (courses[i].grade * courses[i].valuation) + product_sum;      // wie kann ich hier die valuation einfügen? er findet sie nicht!
        printf("%d", product_sum); // nur zur Kontrolle
...
int get_grades(course x)                          // function to get the grades of the user and print it
{
    get_short("Note", 0, 15);
    // calculate_final(x);
}                                           // nein -> man weiß nicht wie groß das array von modulen wird -> Zeiger anlegen -> Speicher vergrößern (Speichererweiterung mit Realloc) -> Schleife
